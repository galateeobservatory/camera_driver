...
impl Hyt221 {
    pub fn new(i2c_address: u16) -> Result<Self, &'static str> {
        ...
        i2c.set_slave_address(i2c_address)
            .map_err(|_| "Failed to set i2c slave address")?;
        Ok(Hyt221 { i2c_device: i2c })
    }
    ...
    pub fn read(&mut self) -> Result<(f32, f32), &'static str> {
        ...
        self.i2c_device
            .write(&[0x00])
            .map_err(|_| "Failed to write to i2c")?; // Write 0 to trigger refresh
        std::thread::sleep(std::time::Duration::from_millis(60));
        self.i2c_device
            .read(&mut buf)
            .map_err(|_| "Failed to read i2c device")?;
